 ...
 *       partner:email:<email> → code  (reverse lookup)
 */
export async function registerPartner({ code, name, email, consent, consentAt }) {
  ...
  const consentTimestamp = consentAt || now;
  const record    = previous
    ? { ...previous, name, consent: true, consentAt: previous.consentAt ?? consentTimestamp, updatedAt: now }
    : { code, name, email: email.toLowerCase(), registeredAt: now, updatedAt: now,
        consent: true, consentAt: consentTimestamp, salesCount: 0, totalEarned: 0 };
  await Promise.all([
 ...
 */
export async function saveLead(type, fields) {
  const id = new Date().toISOString().replace(/[:.]/g, "-") + "-" + Math.random().toString(36).slice(2, 8);
  const record = { id, type, ...fields, receivedAt: new Date().toISOString() };
  await kv('SET', `lead:${type}:${id}`, JSON.stringify(record));
  await kv('LPUSH', `lead:index:${type}`, id);
  return record;
}

export async function listLeads(type, limit = 50) {
  const ids = await kv('LRANGE', `lead:index:${type}`, '0', String(limit - 1));
  if (!Array.isArray(ids) || !ids.length) return [];
  const out = [];
  for (const id of ids) {
    const raw = await kv('GET', `lead:${type}:${id}`);
    if (raw) { try { out.push(JSON.parse(raw)); } catch { /* skip malformed */ } }
  }
  return out;
}
